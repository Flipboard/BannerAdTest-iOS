(function() {
	...
	// Following same pattern as Android - bundle copy of mraid.json.
	// Details on approach in https://github.com/Flipboard/android/blob/master/libs/mraid/src/main/assets/mraid.js

	// Guard against being included multiple times. Some ads use MRAID without declaring
	// it, so we always inject a reference to mraid.js in the client.
	...
	// Some ads cancel touch events in order to implement swipeable areas, which
	// can break flipping when the ad is fullscreen.
	// See: https://jira.flipboard.com/browse/IPH-12835
	// To workaround this, override the native preventDefault() mechanism and
	// disallow for any touch events so that we can guarantee flip gestures get
	// forwarded to the native gesture recognizer.
	var preventDefault = Event.prototype.preventDefault;
	Event.prototype.preventDefault = function() {
		// Disallow touchstart and touchmove
		if (!this.type.match(/^touch(start|move)/)) {
	...
	var LogLevelEnum = mraid.LogLevelEnum = {
		"DEBUG"   : 0,
		"INFO"    : 1,
		"WARNING" : 2,
		"ERROR"   : 3,
	...
	mraid.logLevel = LogLevelEnum.NONE;

	var log = {};

	log.d = function(msg) {
//		if (mraid.logLevel <= LogLevelEnum.DEBUG) {
...
//		}
	...
	log.i = function(msg) {
//		if (mraid.logLevel <= LogLevelEnum.INFO) {
...
//		}
	...
	log.w = function(msg) {
//		if (mraid.logLevel <= LogLevelEnum.WARNING) {
...
//		}
	...
	log.e = function(msg) {
//		if (mraid.logLevel <= LogLevelEnum.ERROR) {
...
//		}
	...
	mraid.createCalendarEvent = function(parameters) {
		...
		if (supportedFeatures[mraid.SUPPORTED_FEATURES.CALENDAR]) {
			// TODO callNative("createCalendarEvent?eventJSON="	+ JSON.stringify(parameters));
		} else {
	...
	mraid.close = function() {
        ...
        window.webkit.messageHandlers.FlipboardMRAIDBridge.postMessage("close");
	};
	...
	mraid.expand = function(url) {
		...
		if (url === undefined) {
            window.webkit.messageHandlers.FlipboardMRAIDBridge.postMessage("expand");
		} else {
		    // TODO callNative("expand?url=" + encodeURIComponent(url));
		}
	};
	...
	mraid.isViewable = function() {
		log.i("mraid.isViewable: " + (isViewable ? "yes" : "no"));
		return isViewable;
	};

	mraid.open = function(url) {
		log.i("mraid.open " + url);
        window.webkit.messageHandlers.FlipboardMRAIDBridge.postMessage("open " + url);
	};

	mraid.playVideo = function(url) {
		log.i("mraid.playVideo " + url);
        window.webkit.messageHandlers.FlipboardMRAIDBridge.postMessage("playVideo " + url);
	};
	...
	mraid.setExpandProperties = function(properties) {
		...
		if (expandProperties.useCustomClose !== oldUseCustomClose) {
            window.webkit.messageHandlers.FlipboardMRAIDBridge.postMessage("useCustomClose " + expandProperties.useCustomClose);
		}
	...
	mraid.setOrientationProperties = function(properties) {
		...
		// TODO callNative("setOrientationProperties?" + params);
	};
	...
	mraid.setResizeProperties = function(properties) {
		...
		// TODO callNative("setResizeProperties?" + params);
	...
	mraid.storePicture = function(url) {
		...
		if (supportedFeatures[mraid.SUPPORTED_FEATURES.STOREPICTURE]) {
			// TODO callNative("storePicture?url=" + encodeURIComponent(url));
		} else {
	...
	mraid.fireReadyEvent = function() {
		// Guard against mutliple ready events - can cause some ads to re-layout incorrectly.
	...
	function fireEvent(event) {
		...
		if (eventListeners) {
			var eventListeners = eventListeners.slice(0);
	...
	// MRAID API ready to use once we have a document.
	...
	if (document.readyState === 'complete') {
		...
		document.addEventListener('DOMContentLoaded', documentReady, true);
	}

	// Notify MRAIDView when ad content is completely loaded(resources are loaded an executed)
	// Android WebView "onPageFinished" callback is sometimes called too soon before javascript is done execution.
	window.addEventListener("load", function(e){
        window.webkit.messageHandlers.FlipboardMRAIDBridge.postMessage("pageDidLoad");
	})
 
	console.log("MRAID object loaded");

	// Listen for postMessage events from embedded ad.
	window.addEventListener("message", function(evt) {
		console.log("handleMessage - data: " + evt.data);
	}, false);
